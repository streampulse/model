...
#' \url{data.streampulse.org} to download data via a browser.
#'
...
#'   Full list of regions and site codes at
#'   \url{data.streampulse.org/sitelist}.
#' @param startdate string of the format 'YYYY-MM-DD' representing the first
#'   day of data to be
...
#'   \url{data.streampulse.org/visualize}. This will populate the date
#'   input field with the first and last available dates.
#' @param enddate string of the format 'YYYY-MM-DD' representing the last day of
#'   data to be
#'   requested. All available records for the specified day will be requested.
#'   If data coverage does not extend this far in time, the argument
...
#'   at \url{data.streampulse.org/visualize}. Omit this argument to
#'   request all variables potentially useful for metabolism modeling.
#' @param flags logical. If \code{TRUE}, flag information (flag
#'   type and comments) will be included in output.
#' @param token A unique alphanumeric string for each registered user of
#'   data.streampulse.org. Only necessary for accessing embargoed data.
...
#' @export
#' @examples
#' streampulse_data = request_data(sitecode='NC_Eno',startdate='2016-06-10',
#'     enddate='2016-10-23')
