...
lim=0
df = input_data #extracted function contents below for simplicity. had to change name here
...
for(g in unique(group)){
    ...
    if( length(mm)>=lim && length(mm)<=maxspan_days ){
        ...
        pp = list(missing=select(missing,-date,-time), similar=select(similar,-date,-time), index=select(similar,date,time))
