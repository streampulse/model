...
points(time(tm)[big_outliers], tm[big_outliers], col='red', pch=20)
mtext('+/- 4 SD', 2, 3, outer=FALSE, font=2)
# par(new=TRUE)
plot(diffs, col='steelblue3', lwd=2, xaxt='n')
mtext('diff', 2, 3, outer=FALSE, font=2)
# mtext('day', 1, 3, font=2)
...
points(time(tm)[pos_jumps], diffs[pos_jumps], col='orange', pch=20, cex=1.5)
points(time(tm)[neg_jumps], diffs[neg_jumps], col='orange', pch=20, cex=1.5)
# first_big_jump = min(c(pos_jumps, neg_jumps))
jump_inds = sort(c(pos_jumps, neg_jumps))
# jump_inds_s = sort(jump_inds)
library(accelerometry)
...
posNeg_jump_pairs = sort(unique(c(keep, as.vector(runs[!lr,2:3]))))
outlier_inds = jump_inds[posNeg_jump_pairs]
points(time(tm)[outlier_inds], diffs[outlier_inds], col='red', pch=4, cex=1)
...
# plot(diffs, col='steelblue3', lwd=2)#, xlim=c(6.5,7))
# points(time(tm)[outlier_ts+1], diffs[outlier_ts], col='orange', pch=20, cex=2)
plot(tm, col='darkgreen', lwd=2)#, xlim=c(6.5,7))
mtext('day', 1, 3, font=2)
mtext('all outl', 2, 3, font=2)
points(time(tm)[outlier_ts], tm[outlier_ts], col='red', pch=20, cex=1)
...
#commented so that code above can be run with ctrl+alt+e
